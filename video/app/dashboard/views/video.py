...
from app.libis.base_render import render_to_response
...
class ExternaVideo(View):
    ...
    def post(self, request):
        ...
        result = check_and_get_video_type(NationalityType, nationality, '非法的视频国籍~')
        if result.get('code') != 0:
            return redirect('{}?error={}'.format(reverse_path, result['msg']))
        if not video_id:
            try:
...
class VideoSubView(View):
    ...
    def post(self, request, video_id):
        ...
        try:
            ...
            return redirect('{}?error={}'.format(url_format, '操作失败~'))
